...
async fn main() {
    ...
    let config = Config {
        packages,
        tencent_package_url:
            "https://mirrors.cloud.tencent.com/repository/composer/p/%package%.json",
    ...
    let app = Router::new()
        ...
        .route(
            "/p2/*package_path",
            get(package_meta).with_state(config.clone()),
        )
        .route(
        ...
        .route(
            "/packages.json",
            get(packages_meta).with_state(config.packages.clone()),
        );
    ...
    match check_package_in_white_list(&package, &config.package_white_list) {
        ...
        false => {
            headers.insert(
                ...
                HeaderValue::try_from(get_package_url(package, config.tencent_package_url))
                    .unwrap(),
            );
